...
description: |
  WordPressのセキュリティ対策を優先順位つきで解説。狙われる4つの入口、自動更新を止めていて本体の欠陥を突かれた実例、初心者が自分を締め出す失敗例、被害時にやってはいけないことまで、実際の改ざん復旧の現場から。
pubDate: '2026-07-30'
...
私が改ざん被害を調査したとき、一つの侵入口に**1日あたり1万件を超えるアクセス**が記録されていました。そして最も衝撃的なのが、侵入から被害が完成するまでの速さです。攻撃が始まってから、攻撃者が不正プログラムを設置し、動作テストまで終えるのに要した時間は、**わずか2分半**でした。「気づいてから対応すればいい」という考えは成り立ちません。
...
<aside class="callout callout-note"><p><strong>2026年9月24日 訂正</strong></p><p>公開時、私が対応した事案の侵入経路を「REST APIの一括処理を使ったパスワードの総当たり」と書いていました。その後、別の事案を調べる中で、WordPress本体の欠陥を突かれた可能性が高いと分かったため、該当する箇所を書き直しました。対策の優先順位にも「本体とプラグインを最新に保つ」を加えています。</p></aside>
...
制作会社やサイトの管理者が、WordPressの自動更新をあえて止めていることがあります。更新で表示が崩れるのを避けるためです。私たちも、以前はそうしていました。

ところが、**私が対応した改ざん事案の原因は、WordPress本体の欠陥でした。** 直した版は、攻撃のちょうど1週間前に公開されていました。自動更新を止めていたために、古いバージョンのまま狙われたのです。

欠陥が見つかると、その情報は世界中に広まり、自動で試すプログラムがすぐに出回ります。**更新しないでいる期間は、そのまま「開いている期間」になります。** 表示の崩れは直せば済みますが、乗っ取られたサイトの復旧には、その何倍もの手間がかかります。
...
これが、私が対応した事案で実際に使われた入口です。そして、いま最も説明が必要な場所だと思っています。
...
ところが、この機能には**WordPress本体の欠陥**がありました（2026年7月に直した版が公開）。細工したリクエストを送ると、ログインしていない人でも、中のデータを書き換えられてしまうものです。

実際のログでは、攻撃者はこの入口に約1分間で25回リクエストを送っていました。その直後に管理者のパスワードが書き換わり、攻撃者はその**新しいパスワードで、玄関から堂々とログイン**しています。パスワードは当てられたのではなく、書き換えられていたのです。

これは、ログイン試行回数の制限やパスワードの強さでは防げません。**玄関にどれだけ頑丈な鍵をかけても、通用口の錠そのものが壊れていた**、というのが実際に起きたことです。錠を直す方法は、本体を最新にすること。使っていないなら、通用口ごと閉じておけばさらに安全です。
...
攻撃が始まってから、攻撃者がサーバー内に不正プログラムを設置し、その動作テストまで終えるのに要した時間は**わずか2分半**でした。アクセスログを秒単位で追って、初めて分かったことです。「気づいてから対応すればいい」が成り立たない理由がここにあります。気づいた時点では、すでに終わっています。
...
### 【最優先】WordPress本体とプラグインを最新に保つ

順位をつける前に、すべての土台になることが一つあります。**WordPress本体とプラグインを、最新の状態に保つこと**です。

前の章でお話しした通り、私が対応した事案は、本体の欠陥を突かれたものでした。直した版はすでに出ていたので、更新さえしていれば防げた攻撃です。

いまのWordPressは、管理画面から自動更新をオンにできます。本体は「ダッシュボード」→「更新」の画面から、プラグインは「プラグイン」一覧の各行にある「自動更新を有効化」から切り替えられます。

一つ注意があります。制作会社がテーマの中で自動更新を止めていて、画面で切り替えても効かないことがあります。自分のサイトがどうなっているか分からなければ、制作を依頼した会社に「自動更新は動いていますか」と一言聞いてみてください。

更新で表示が崩れることは、まれにあります。それでも、崩れた表示は直せば済みます。乗っ取られてからの復旧に比べれば、手間も費用もずっと小さく済みます。
...
次に、かなりの範囲をまとめてカバーできることです。WAF（Webアプリケーションファイアウォール）は、サイトの前に立って不審なアクセスを止めてくれる仕組みです。先ほど説明した勝手口からの総当たりや、通用口を狙った不審なリクエストも含めて、多くの不正アクセスをここで防ぎます。
...
前章で説明した「通用口」は、ログイン画面を見張るタイプのセキュリティプラグインでは守れません。私の事案は本体の欠陥を突かれたものだったので、根本の対策は本体の更新です。そのうえで、使っていない一括処理の入口をサーバー側の設定ファイルで塞いでおくと、二重の守りになります。詳しくは次の章で説明しますが、**プラグインだけでは守り切れない領域がある**ことを頭に入れておいてください。
...
私が侵入経路を秒単位で追えたのは、発覚した当日にログを確保できたからです。実は、そのときの私は、このログを「パスワードの総当たり」と読み違えていました。2か月後、別の事案を調べる中で本体の欠陥に気づき、同じログを読み直して訂正できました。**ログが残っていたから、あとから間違いに気づけた**のです。
